...
- **Interface**: CLI only — no web UI, no server, no open ports.
- **Hardware**: CUDA, MPS, or CPU (Auto-detected)

## Architecture Overview
- `app.py`: Entry point. Parses args and hands off to the CLI.
- `core/engine.py`: The heart of synthesis. Uses `get_model()` for lazy loading and hardware fallback.
- `core/text.py`: Normalization, chapter regex logic, and the custom pronunciation dictionary parser.
- `core/voices.py`: The `CHOICES` map of display labels to voice IDs.
- `cli.py`: Argument parsing and the batch synthesis loop (`run_cli`).
...
- **Pathing**: Default export path is `~/Documents/Kokoro_Exports`.

## Common Tasks
- **Adding a Voice**: Update `CHOICES` in `core/voices.py`.
- **Modifying Normalization**: Edit `normalize_text` in `core/text.py`.
- **Extending CLI**: Update `build_parser`/`run_cli` in `cli.py`.
